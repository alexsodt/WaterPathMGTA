...
{
	int headerLength = sizeof(DCDheader1) + curHeader.header1.ntitl * sizeof(char) + sizeof(int) * 4;

	int frameLength = 0;

	if( curHeader.header1.qcrys )
	{
		frameLength += 6 * sizeof(double) + sizeof(int);
	}
	int natom = curHeader.natom;

	frameLength += sizeof(float) * natom * 3;
	frameLength += (6+1) * sizeof(int);

	size_t offset = (size_t)headerLength + (size_t)frameLength * (size_t)f;

	fseeko( theFile, offset, SEEK_SET );	
}
